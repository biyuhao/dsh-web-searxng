/**
 * Probe round execution (host side of the card ↔ host channel).
 *
 * The card writes a probe request into the entry Config's `probe*` fields
 * (requestId LAST); the host fans targets out in parallel and writes results
 * back via `settings.update`. All values are flat scalars; lists ride
 * JSON-encoded strings so any settings backend round-trips them.
 *
 * Probes run host-side because the browser cannot use the host egress proxy
 * (socks5h) and community instances send no CORS headers; there is no
 * host↔client RPC for third-party plugins (`ctx.remote` property access is
 * gated by fiber injects they cannot satisfy). Passwords are never logged.
 * A failed probe is a normal outcome (`ok: false` per target); only a
 * malformed request sets a round-level `probeError`.
 */
...
/**
 * Run one probe round and write the result back (never throws). `cfg` is the
 * committed protocol snapshot; `writeBack` merges the result fields into the
 * same entry's config. The guard in index.ts matches `probeResultId` against
 * the answered id, so this write-back never re-triggers the round.
 */
